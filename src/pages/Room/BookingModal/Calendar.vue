        ...
        <div class="calendar__date">
            ...
            <div class="calendar__days">
                <ul v-for="(week, index) in calendarArray" :key="index" :id="`week${index}`">
                    <li v-for="(day, index) in week" :key="index" @click="selectDate(day)" :class="`${type}${currentMonth}`" :id="`${day}`" >
                        {{ day }}
                    </li>
                </ul>
...
export default {
    ...
    methods: {
        requestMonthBookings(month=this.month, year=this.year) {
            ...
            this.$store.dispatch('getRoomBookingsPerMonth', args).then(() => {
                this.bookingsCalendarBackground();
            });
        },
        bookingsCalendarBackground() {
            const daysContainers = document.querySelectorAll(`.calendar__days ul .${this.type}${this.currentMonth}`);
            daysContainers.forEach(day => {
                day.style.color = '#626262';
                day.style.cursor = 'pointer';
                day.style.textDecoration = 'none';
                day.classList.add('li-hover');
            });
